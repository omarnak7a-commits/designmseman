...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  const icons: Record<ToastType, React.ReactNode> = {
    success: (
      <svg viewBox="0 0 20 20" fill="currentColor" className="w-5 h-5 text-[#16a34a]">
    ...
    error: (
      <svg viewBox="0 0 20 20" fill="currentColor" className="w-5 h-5 text-[#dc2626]">
    ...
    info: (
      <svg viewBox="0 0 20 20" fill="currentColor" className="w-5 h-5 text-[#2563eb]">
    ...
    <ToastContext.Provider value={{ toast }}>
      ...
      <div className="fixed bottom-4 right-4 z-[100] flex flex-col gap-2 pointer-events-none">
        {toasts.map((t) => (
          <div
            key={t.id}
            className="flex items-center gap-3 bg-white border border-[#e2e8f0] rounded-xl shadow-lg px-4 py-3 min-w-64 max-w-sm pointer-events-auto"
